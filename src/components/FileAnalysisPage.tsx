...
export const FileAnalysisPage: React.FC<FileAnalysisPageProps> = ({ onSaveHistory }) => {
  ...
  const processRows = async (rows: any[], fName: string) => {
    ...
    setAnalysisMode(hasLabels ? 'labeled' : 'unlabeled');
      ...
      <div className="card">
        ...
        <div className="mode-box">
          <button
            ...
            onClick={() => setAnalysisMode('unlabeled')}
          >
            Chưa biết nhãn
            <br />
            <small style={{ color: 'inherit' }}>Phân loại</small>
          </button>
          <button
            id="labeledMode"
            className={`mode-btn ${analysisMode === 'labeled' ? 'active' : ''}`}
            onClick={() => setAnalysisMode('labeled')}
          >
            Có nhãn
            <br />
            <small style={{ color: 'inherit' }}>Kiểm tra dự đoán</small>
          </button>
        </div>
        ...
        {results.length > 0 && (
          <div id="fileResult" style={{ marginTop: '20px' }}>
            {/* Thông tin tệp tin & chế độ */}
            <div style={{ marginBottom: '20px', fontSize: '14px', color: '#1f2937', lineHeight: '1.6' }}>
              <div><b>File:</b> {fileName}</div>
              <div><b>Chế độ:</b> {analysisMode === 'labeled' ? 'Có nhãn – Kiểm tra dự đoán' : 'Chưa biết nhãn – Phân loại'}</div>
              <div><b>Số mẫu hợp lệ:</b> {results.length}</div>
            </div>
            ...
            {analysisMode === 'labeled' ? (
              <>
                <div className="overview-grid">
                  ...
                  <div className="stat-card">
                    <div className="stat-title">Accuracy (có nhãn)</div>
              ...
              <div className="overview-grid">
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-number">{counts.setosa}</div>
                </div>
                <div className="stat-card">
                  <div className="stat-title">SVM → Versicolor</div>
                  <div className="stat-number">{counts.versicolor}</div>
                </div>
                <div className="stat-card">
                  <div className="stat-title">SVM → Virginica</div>
                  <div className="stat-number">{counts.virginica}</div>
                </div>
              </div>
            )}
            ...
            <h3 style={{ marginTop: '24px', fontSize: '16px', fontWeight: 600, color: '#1f2937' }}>
              {analysisMode === 'labeled' ? '📊 Phân bố theo kết quả SVM' : '📊 Phân bố kết quả SVM'}
            </h3>
                ...
                <div key={sp} className="flower-summary">
                  ...
                  <div className="flower-summary-content">
                    ...
                    <div className="progress">
                      <div className="progress-bar" style={{ width: `${pct}%`, backgroundColor: '#475569' }}></div>
                    </div>
                  </div>
                </div>
              );
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '24px', flexWrap: 'wrap', gap: '10px' }}>
              <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 600, color: '#1f2937' }}>
                {analysisMode === 'labeled' ? '🔎 So sánh SVM với nhãn thật' : '📋 Bảng chi tiết kết quả phân loại'}
              </h3>
                    ...
                    <tr key={idx}>
                      ...
                      {analysisMode === 'labeled' && (
                        <td className={row.correct ? 'correct' : 'wrong'}>
                          {row.correct === null ? '—' : row.correct ? '✓ Đúng' : '✗ Sai'}
                        </td>
                      )}
                    </tr>
                  ))}
